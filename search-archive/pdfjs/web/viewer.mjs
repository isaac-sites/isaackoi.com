...
const PDFViewerApplication = {
  ...
  async run(config) {
    ...
    file = maybeProxyLocalFileURL(file);
...
PDFPrintServiceFactory.initGlobals(PDFViewerApplication);
{
  ...
  const TRUSTED_FILE_ORIGINS = new Set(["https://files.afu.se"]);
  var maybeProxyLocalFileURL = function (file) {
    const viewerURL = URL.parse(window.location);
    ...
    const proxyURL = new URL("/pdf-proxy", window.location);
    proxyURL.searchParams.set("url", fileURL.href);
  ...
  var validateFileURL = function (file) {
    ...
    if (TRUSTED_FILE_ORIGINS.has(fileOrigin)) {
      return;
    }
